...
//    println(graph.nodes)
    ...
    for (i <-0 until nodes.length){
      //      val FrequencyofNode=ObjectArrayEdge.toList.count(_==nodes(i).id).toDouble
      ...
      //      fileWriter.write(nodes(i).id+","+nodes(i).group+","+nodes(i).state.pos.x.toString+","+nodes(i).state.pos.y.toString+","+"0"+","+ObjectArrayEdge.toList.count(_==nodes(i).id).toDouble/(productedge*2)+","+"\n")
